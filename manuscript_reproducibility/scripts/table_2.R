...
##* measurement, HMM transit prob, Negative to positive
#######################################################

HMM_n2p_tb <- Transition_tb %>%
  filter(HMM == "neg2pos", model == "gaussian") %>%
  inner_join(pat.dat, by = c("sample" = "slide_id"))

fit_hmm_n2p <- coxph(Surv(time = survival_time, event = dead) ~ mean + sd + kurtosis + skewness + K +
