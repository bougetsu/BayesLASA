...
#* read the raw simulated data and output of BayesLASA, ALDUQ and convex hull
#***********************************************
...
ffs <- c(
  "Normal_4_equil_FALSE_pn_150_seed_30",
...
for (fname in ffs) {
  ...
  landmarks <- c(
    landmarks,
    list(cbind.data.frame(pc[L_true, ],
      Method = "True landmarks", loc = "Lamdmarks",
  ...
  landmarks <- c(
    landmarks,
    list(cbind.data.frame(pc[L_ppm, ],
      Method = "BayesLASA (PPM)", loc = "Lamdmarks",
  ...
  for (j in 1:nrow(df_cred_map)) {
    if ((df_cred_map$lwr[j] <= df_cred_map$upr[j]) & (df_cred_map$upr[j] - df_cred_map$lwr[j] < 90)) {
      landmarks_cred[[j]] <- data.frame(
        x = pc[df_cred_map$lwr[j]:df_cred_map$upr[j], 1],
  ...
  landmarks <- c(
    landmarks,
    list(cbind.data.frame(pc[L_ppi, ],
      Method = "BayesLASA (MAP)", loc = "Lamdmarks",
      cloc = "Lamdmarks", K = num, cmethod = "BayesLASA"
    ))
  )
  ...
  landmarks <- c(
    landmarks,
    list(cbind.data.frame(pc[L_jasa_post, ],
      Method = "ALDUQ", loc = "Lamdmarks",
  ...
  landmarks <- c(
    landmarks,
    list(cbind.data.frame(pc[L_chull, ],
      Method = "Convex Hull", loc = "Lamdmarks",
...
landmarks2 <- landmarks %>%
  rbind(landmarks_cred_list) %>%
  mutate(
    ccmethod = factor(Method, levels = c("True landmarks", "BayesLASA (MAP)", "BayesLASA (PPM)", "ALDUQ", "Convex Hull")),
...
#* plot
p_example <- ggscatter(
  data = subset(landmarks2, cloc == "Lamdmarks"), x = "x", y = "y", color = "Method", shape = "Method",
  size = 0.01,
  ...
  geom_polygon(
    data = as.data.frame(pcs), aes(x = x, y = y), fill = NA, linetype = "solid",
    size = 0.5, color = "grey70"
  ) +
  geom_point(data = subset(landmarks2, cloc == "Lamdmarks"), aes(x = x, y = y, color = Method, shape = Method), size = 2) +
  geom_path(
    data = subset(landmarks2, cloc == "Credible"), aes(x = x, y = y, group = loc, color = Method),
    size = 2, alpha = 0.3
  ) +
  scale_shape_manual(values = c(9, 15, 17, 19)) +
  theme(
    strip.text = element_text(size = 12, colour = "grey50"),
