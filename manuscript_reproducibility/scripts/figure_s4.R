#************************************************
#* Fig.4. Compare performance of different method on simulated study
#* read the raw data and output of BayesLASA, ALDUQ and convex hull on simulated data
#* calculated MCC
#* do violin and boxplot of MCCs
#***********************************************
...
p_MCC <- df_plot %>%
  ...
  mutate(
    cmethod = ifelse(str_detect(method, "chull"), "Convex Hull",
  ...
  ggviolin(
    x = "cmethod", y = "value", color = "Method",
    facet.by = c("K", "pn"),
...
ggsave(p_MCC, file = file.path(fig.output, paste0("figure_s4.pdf")), width = 12, height = 9)
