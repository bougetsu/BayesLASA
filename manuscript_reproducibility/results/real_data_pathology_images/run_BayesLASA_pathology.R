...
output = "manuscript_reproducibility/data/real_data_pathology_images/BayesLASA/"
...
foreach(f = ff) %dopar% {
  ...
  res = foreach(j = 1:4) %dopar% {
    ...
    MCMC_shape(dat,  iter = n*fold,  estK = est.K, gamma_i = gamma_i, updateGp = updateG.p,
...
for(f in ff){
  ...
  for(k in 1:length(L_ppm)){
    ...
    for(ss in 1:nrow(segs)){
      ...
      t = try( {fit.mod1 = fit(mod1)}, silent=T)
      if (!inherits(t, "try-error")) {
        ...
        if(length(transition_mat) < k){
          ...
          transition_mat[[k]] = rbind(transition_mat[[k]],
                                      transition %>% mutate(sample = sample, shape = k, BetaSigma = bs, model = c("sign","gaussian"),  segs = ss))
          coeffs[[k]] = as.data.frame(coeff) %>% mutate(sample = sample, shape = k, BetaSigma = bs, status = c("-1","1"),  segs = ss) %>% rbind(coeffs[[k]])
